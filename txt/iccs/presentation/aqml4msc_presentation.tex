...
%  ICCS TITLE PAGE
\author[Sebastian Dziura]{Tomasz Rybotycki\inst{1,2,3} \and \underline{Sebastian Dziura}\inst{1} \and Piotr Gawron\inst{1,2}%
...
\begin{frame}[t,fragile]{{Quantum Embedding}}
	...
	\begin{columns}[t,onlytextwidth]
		...
		\only<2>{
			\begin{center}
				\fbox{$n$ bits $\rightarrow$ $n$ qubits}
			\end{center}
		...
		\only<3>{
			\begin{center}
				\fbox{$n$ features $\rightarrow$ $n$ qubits}
			\end{center}
			\textbf{Classical data:}
			$$ \mathbf{x}  = \begin{bmatrix} \frac{1}{4} & \frac{1}{3} & \frac{1}{2} \end{bmatrix} $$
			\textbf{Quantum state:}
			$$ \ket{\psi} = R_x\left(\frac{\pi}{4}\right) \ket{0} \otimes R_x\left(\frac{\pi}{3}\right) \ket{0} \otimes R_x\left(\frac{\pi}{2}\right) \ket{0} $$
		}
		\only<4>{
			\begin{center}
				\fbox{$2^n$ features $\rightarrow$ $n$ qubits}
			\end{center}
			\textbf{Classical data:}
...
\begin{frame}[fragile]{{Variational Layers}}
	\begin{tikzpicture}[remember picture, overlay]
		...
		{\includegraphics[width=8cm]{fig/basic_entangler.jpg}};

		% Image 4: Top Right
		\node<4-> at ([shift={(-4cm,-3cm)}]current page.north east)
		{\includegraphics[width=8cm]{fig/simplified_two_design.jpg}};

		% Image 5: Bottom Left
		\node<5-> at ([shift={(4cm,2.5cm)}]current page.south west)
		{\includegraphics[width=8cm]{fig/layer_rnd.png}};
	\end{tikzpicture}
	\let\thefootnote\relax\footnotetext{Sources: \url{https://pennylane.ai}}
\end{frame}
...
\begin{frame}[fragile, c]{{Tools}}
	\vfill
	\begin{center}
		\includegraphics[width=0.8\textwidth]{fig/pennylane-logo.png}

		\vspace{3em}
		
		\includegraphics[width=0.7\textwidth]{fig/pytorch-logo.png}
	\end{center}
	\vfill
	\let\thefootnote\relax\footnotetext{Sources: \url{https://pennylane.ai}, \url{https://pytorch.org}}
\end{frame}
...
\begin{frame}[fragile, c]{{Next steps: real hardware}}
	\begin{columns}[c]
		\begin{column}{0.6\textwidth}
			\centering
			\includegraphics[width=0.7\linewidth, height=3.5cm, keepaspectratio]{fig/VLQ_logo.jpg}
						
			\vspace{0.5cm}
			
			\includegraphics[width=1\linewidth, height=3.5cm, keepaspectratio]{fig/IQM_Star_24.png}
			
		\end{column}
		\begin{column}{0.4\textwidth}
			\includegraphics[width=1\linewidth, height=10cm, keepaspectratio]{fig/VLQ.png}
			
		\end{column}
	\end{columns}
	\let\thefootnote\relax\footnotetext{Sources: \url{https://www.it4i.cz/}}
\end{frame}


\begin{frame}[fragile]{{Ending}}
	\centering{\huge Thank you for attention}
